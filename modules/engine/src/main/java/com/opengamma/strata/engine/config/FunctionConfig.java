...
public final class FunctionConfig<T extends CalculationTarget> implements ImmutableBean {
   ...
   */
  private static final FunctionConfig<? extends CalculationTarget> DEFAULT =
  ...
  public static <T extends CalculationTarget> FunctionConfig<T> defaultFunction() {
    return (FunctionConfig<T>) DEFAULT;
  }
  ...
  private static boolean isArgumentCompatible(Class<?> parameterType, Class<?> argumentType) {
    ...
    return isArgumentCompatible(convertToBoxedType(parameterType), argumentType);
  }

  /**
   * Returns the boxed type of the argument if it is primitive, else returns the argument.
   *
   * @param type  a type
   * @return the boxed type of the argument if it is primitive, else returns the argument
   */
  private static Class<?> convertToBoxedType(Class<?> type) {
    if (!type.isPrimitive()) {
      return type;
    }
