...
/**
 * A parameterized interest rate swap leg.
 * <p>
...
public final class RateCalculationSwapLeg
  ...
  public ExpandedSwapLeg expand() {
    ...
    ImmutableList<RateAccrualPeriod> accrualPeriods = calculation.expand(resolvedAccruals, resolvedPayments);
    ImmutableList<RatePaymentPeriod> payPeriods = paymentSchedule.createPaymentPeriods(
  ...
  private List<PaymentEvent> createEvents(ImmutableList<RatePaymentPeriod> payPeriods) {
    Currency currency = notionalSchedule.getCurrency();
    // TODO: fx reset exchange
    List<PaymentEvent> events = new ArrayList<>();
    if (notionalSchedule.isInitialExchange()) {
    ...
    if (notionalSchedule.isIntermediateExchange()) {
    ...
    if (notionalSchedule.isFinalExchange()) {
