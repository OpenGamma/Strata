...
public final class InflationRateCalculation
   ...
   * <p>
   * In most of the cases, lags are standard so _conventionalMonthLag=_monthLag.
   */
  ...
  private RateObservation createRateObservation(SchedulePeriod period) {
    ...
    if (isInterpolated) {
      ...
      referenceStartDates[0] = period.getStartDate().minusMonths(monthLag).with(TemporalAdjusters.lastDayOfMonth());
      referenceStartDates[1] = referenceStartDates[0].plusMonths(1).with(TemporalAdjusters.lastDayOfMonth());
      referenceEndDates[0] = period.getEndDate().minusMonths(monthLag).with(TemporalAdjusters.lastDayOfMonth());
      ...
      return InflationInterpolatedRateObservation.of(index, referenceStartDates, referenceEndDates, weight);
    }
    ...
    return InflationMonthlyRateObservation.of(index, referenceStartDate, referenceEndDate);
  }
  //------------------------- AUTOGENERATED START -------------------------
  ///CLOVER:OFF
  /**
