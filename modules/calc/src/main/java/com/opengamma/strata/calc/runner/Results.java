...
public final class Results implements ImmutableBean {
  ...
  //-------------------------------------------------------------------------
  /**
   * Processes a successful result by applying a function that alters each value.
   * <p>
   * This operation allows post-processing of result values.
   * The specified function represents a conversion to be performed on each value.
   *
   * @param function  the function to transform the value with
   * @return the new results
   */
  public Results map(Function<Result<?>, Result<?>> function) {
    List<Result<?>> mappedResults = items.stream()
        .map(function)
        .collect(toImmutableList());
    return new Results(rowCount, columnCount, mappedResults);
  }
