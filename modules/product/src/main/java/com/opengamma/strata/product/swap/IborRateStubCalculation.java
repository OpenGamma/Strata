...
public final class IborRateStubCalculation
  ...
  void collectCurrencies(ImmutableSet.Builder<Currency> builder) {
    getKnownAmount().ifPresent(amt -> builder.add(amt.getCurrency()));
    getIndex().ifPresent(idx -> builder.add(idx.getCurrency()));
    getIndexInterpolated().ifPresent(idx -> builder.add(idx.getCurrency()));
  }

  // collect the indices
  void collectIndices(ImmutableSet.Builder<Index> builder) {
    getIndex().ifPresent(idx -> builder.add(idx));
    getIndexInterpolated().ifPresent(idx -> builder.add(idx));
  }
