...
import static com.opengamma.basics.BasicProjectAssertions.assertThat;
...
import static com.opengamma.basics.currency.Currency.SEK;
...
public class FxMatrixTest {
  ...
  public void emptyMatrixCanHandleTrivialRate() {
    ...
    assertThat(matrix.getRate(USD, USD)).isEqualTo(1.0);
  }

  public void emptyMatrixCannotDoConversion() {
    FxMatrix matrix = FxMatrix.builder().build();
    assertThat(matrix.getCurrencies()).isEmpty();
    assertThrows(() -> matrix.getRate(USD, EUR), IllegalArgumentException.class);
  }
  ...
  public void singleRateMatrix() {
    ...
    assertThat(matrix.getRate(GBP, USD)).isEqualTo(1.6);
    assertThat(matrix.getRate(USD, GBP)).isEqualTo(0.625);
  }
  ...
  public void matrixCalculatesCrossRates() {
    ...
    assertThat(matrix.getRate(GBP, USD)).isEqualTo(1.6);
    assertThat(matrix.getRate(USD, GBP)).isEqualTo(1 / 1.6);
    assertThat(matrix.getRate(EUR, USD)).isEqualTo(1.4);
    assertThat(matrix.getRate(USD, EUR)).isEqualTo(1 / 1.4);
    assertThat(matrix.getRate(EUR, GBP)).isEqualTo(1.4 / 1.6, TOL);
    assertThat(matrix.getRate(GBP, EUR)).isEqualTo(1.6 / 1.4, TOL);
    assertThat(matrix.getRate(EUR, CHF)).isEqualTo(1.2);
  }
  ...
  public void ratedCanBeUpdatedAndAddedViaBuilder() {
    ...
    assertThat(matrix1.getRate(GBP, USD)).isEqualTo(1.5);
    ...
    assertThat(matrix2.getRate(GBP, USD)).isEqualTo(1.6);
    assertThat(matrix2.getRate(EUR, USD)).isEqualTo(1.4);
  }
...
EUR {1.4, 0.933, 1.0}}
    ...
    assertThat(matrix1.getRate(GBP, USD)).isEqualTo(1.5);
    assertThat(matrix1.getRate(EUR, USD)).isEqualTo(1.4);
    assertThat(matrix1.getRate(EUR, GBP)).isEqualTo(1.4 / 1.5, TOL);
    ...
    // EUR/USD is affected
    assertThat(matrix2.getRate(EUR, USD)).isEqualTo(1.4 * (1.6 / 1.5), TOL); // = 1.49333
    // but EUR/GBP is not
    assertThat(matrix2.getRate(EUR, GBP)).isEqualTo(1.4 / 1.5, TOL);  // = 0.9333

    // The rate we updated
    assertThat(matrix3.getRate(GBP, USD)).isEqualTo(1.6);

    // As matrix3 update was restating GBP wrt USD, there is
    // no effect on EUR/USD
    assertThat(matrix3.getRate(EUR, USD)).isEqualTo(1.4);
    // but there is an effect on EUR/GBP
    assertThat(matrix3.getRate(EUR, GBP)).isEqualTo((1.4 / 1.5) * (1.5 / 1.6), TOL); // = 0.875
  }
  ...
  public void rateCanBeUpdatedWithDirectionSwitched() {
    ...
    assertThat(matrix1.getRate(GBP, USD)).isEqualTo(1.6);
  ...
  public void streamPairsToMatrix() {
    ...
    assertThat(matrix.getRate(GBP, USD)).isEqualTo(1.616);
    assertThat(matrix.getRate(EUR, USD)).isEqualTo(1.414);
  }
  ...
  public void addMultipleRatesSingle() {
    ...
    assertThat(matrix.getRate(GBP, USD)).isEqualTo(1.6);
    assertThat(matrix.getRate(USD, GBP)).isEqualTo(1 / 1.6);
    assertThat(matrix.getRate(EUR, USD)).isEqualTo(1.4, TOL);
    assertThat(matrix.getRate(USD, EUR)).isEqualTo(1 / 1.4, TOL);
    assertThat(matrix.getRate(EUR, GBP)).isEqualTo(1.4 / 1.6, TOL);
    assertThat(matrix.getRate(GBP, EUR)).isEqualTo(1.6 / 1.4, TOL);
    assertThat(matrix.getRate(EUR, CHF)).isEqualTo(1.2);
  }
  ...
  public void mergeAddsInAdditionalCurrencies() {
    ...
    assertThat(result.getRate(GBP, USD)).isEqualTo(1.6);
    assertThat(result.getRate(GBP, EUR)).isEqualTo(1.6 / 1.4, TOL);

    assertThat(result.getRate(EUR, CHF)).isEqualTo(1.2);
    assertThat(result.getRate(CHF, AUD)).isEqualTo(1.2);

    assertThat(result.getRate(GBP, CHF)).isEqualTo((1.6 / 1.4) * 1.2, TOL);
    assertThat(result.getRate(GBP, AUD)).isEqualTo((1.6 / 1.4) * 1.2 * 1.2, TOL);
  }
