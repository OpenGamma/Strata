...
public class Business252DayCountTest {
  ...
  public void test_factory_name() {
    ...
    assertSame(DayCount.ofBus252(HolidayCalendars.EUTA), test);
  }

  public void test_factory_calendar() {
    DayCount test = DayCount.ofBus252(HolidayCalendars.GBLO);
    assertEquals(test.getName(), "Bus/252 GBLO");
    assertEquals(test.toString(), "Bus/252 GBLO");

    assertSame(DayCount.of("Bus/252 GBLO"), test);
    assertSame(DayCount.ofBus252(HolidayCalendars.GBLO), test);
  }
  ...
  public void test_yearFraction() {
    ...
    for (int i = 0; i < 366; i++) {
      assertEquals(test.yearFraction(date1, date2), HolidayCalendars.EUTA.daysBetween(date1, date2) / 252d);
  ...
  public void test_serialization() {
    assertSerialization(DayCount.ofBus252(HolidayCalendars.EUTA));
  }

  public void test_jodaConvert() {
    assertJodaConvert(DayCount.class, DayCount.ofBus252(HolidayCalendars.EUTA));
  }
