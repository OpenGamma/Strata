 ...
 * select an underlying parameter based on the trade counterparty represented by its Standard ID.
 */
...
public final class TradeCounterpartyCalculationParameter
  ...
  public Optional<CalculationParameter> filter(CalculationTarget target, Measure measure) {
    ArgChecker.isTrue(target instanceof Trade, "target must be a trade for the CounterpartyCalculationParameter");
    Trade trade = (Trade) target;
    Optional<StandardId> idOpt = trade.getInfo().getCounterparty();
    ArgChecker.isTrue(idOpt.isPresent(), "trade must have a counterparty");
