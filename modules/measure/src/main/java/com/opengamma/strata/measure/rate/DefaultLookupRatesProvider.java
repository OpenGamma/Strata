...
final class DefaultLookupRatesProvider
  ...
  public PriceIndexValues priceIndexValues(PriceIndex index) {
    ...
    if (!(curve instanceof NodalCurve)) {
      throw new IllegalArgumentException("Curve must be a NodalCurve: " + index);
    }
    return PriceIndexValues.of(index, getValuationDate(), (NodalCurve) curve, timeSeries(index));
  }
