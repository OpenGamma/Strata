...
final class DefaultSwaptionMarketDataLookup
  ...
  public ImmutableSet<IborIndex> getVolatilityIndices() {
    return getVolatilityIndices(volatilityIds);
  }

  static ImmutableSet<IborIndex> getVolatilityIndices(ImmutableMap<IborIndex, SwaptionVolatilitiesId> volatilityIds) {
  ...
  public ImmutableSet<MarketDataId<?>> getVolatilityIds(IborIndex index) {
    return getVolatilityIds(volatilityIds, index, "swaption");
  }

  static ImmutableSet<MarketDataId<?>> getVolatilityIds(
      ImmutableMap<IborIndex, SwaptionVolatilitiesId> volatilityIds,
      IborIndex index,
      String tradeType) {

    SwaptionVolatilitiesId id = volatilityIds.get(index);
    if (id == null) {
      throw new IllegalArgumentException(msgIndexNotFound(index, tradeType));
    }
  ...
  public FunctionRequirements requirements(Set<IborIndex> indices) {
    return requirements(volatilityIds, indices, "swaption");
  }

  static FunctionRequirements requirements(
      ImmutableMap<IborIndex, SwaptionVolatilitiesId> volatilityIds,
  ...
  public SwaptionVolatilities volatilities(IborIndex index, MarketData marketData) {
    return volatilities(volatilityIds, index, marketData, "swaption");
  }

  static SwaptionVolatilities volatilities(
      ImmutableMap<IborIndex, SwaptionVolatilitiesId> volatilityIds,
      IborIndex index,
    ...
    if (volatilityId == null) {
      throw new MarketDataNotFoundException(msgIndexNotFound(index, tradeType));
    }
    return marketData.getValue(volatilityId);
  }

  //-------------------------------------------------------------------------
  private static String msgIndexNotFound(Index index, String tradeType) {
    return Messages.format(tradeType + " lookup has no volatilities defined for index '{}'", index);
  }
  ...
  public String toString() {
    return "DefaultSwaptionMarketDataLookup{" + "volatilityIds" + '=' + JodaBeanUtils.toString(volatilityIds) + '}';
  }
