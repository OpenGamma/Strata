...
public class FunctionUtilsTest {
  ...
  public void getLowerBoundIndexTest() {
    int i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-2., -1.), -0.);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(1., 2.), -0.);
    assertEquals(i, 0);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(1., 2., 3.), 2.5);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(1., 2., 3.), 2.);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(1., 2., 3.), -2.);
    assertEquals(i, 0);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-2., -1., 0.), -0.);
    assertEquals(i, 2);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-2., -1., 0.), 0.);
    assertEquals(i, 2);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-2., -1., 0.), -0.);
    ...
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-2., -1., -0.), -0.);
    assertEquals(i, 2);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-1., 0., 1.), -0.);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-1., 0., 1.), 0.);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-1., -0., 1.), 0.);
    ...
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-1., -0., 1.), -0.);
    assertEquals(i, 1);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(0., 1., 2.), -0.);
    assertEquals(i, 0);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(0., 1., 2.), 0.);
    assertEquals(i, 0);
    i = FunctionUtils.getLowerBoundIndex(DoubleMatrix1D.of(-0., 1., 2.), 0.);
