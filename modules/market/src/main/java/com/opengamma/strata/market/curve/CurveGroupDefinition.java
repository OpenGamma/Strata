...
public final class CurveGroupDefinition
  ...
  private void validate() {
    ArgChecker.isTrue(!computePvSensitivityToMarketQuote || computeJacobian, 
        "compute PV sensitivity requires Jacobian");
  ...
  public ImmutableList<ResolvedTrade> resolvedTrades(
    ...
    return curveDefinitionsByName.values().stream()
        ...
        .map(node -> node.resolvedTrade(valuationDate, 1.0d, marketData, refData))
        .collect(toImmutableList());
  }
  ...
  public CurveGroupDefinitionBuilder toBuilder() {
    return new CurveGroupDefinitionBuilder(name, entriesByName, curveDefinitionsByName, 
  ...
  //-----------------------------------------------------------------------
  /**
   * Gets flag indicating if the Jacobian matrices should be computed and stored in metadata or not.
  ...
  //-----------------------------------------------------------------------
  /**
   * Gets flag indicating if present value sensitivity to market quotes should be computed and stored in metadata or not.
   * @return the value of the property
   */
