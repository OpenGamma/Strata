...
public class SpreadSensitivityCalculatorTest {
  ...
  static {
    ...
    Builder<CdsCurveNode> nodesBuilder = ImmutableList.builder();
    for (int i = 0; i < NUM_MARKET_CDS; i++) {
      QuoteId quoteId = QuoteId.of(StandardId.of("OG", PAR_SPD_DATES[i].toString()));
      CdsCurveNode node =
          CdsCurveNode.ofParSpread(DatesCdsTemplate.of(VALUATION_DATE, PAR_SPD_DATES[i], CDS_CONV), quoteId, LEGAL_ENTITY);
    ...
    ImmutableList<CdsCurveNode> nodes = nodesBuilder.build();
    CreditRatesProvider rates = CreditRatesProvider.builder()
        ...
        .build();
    CREDIT_CURVE = BUILDER.calibrate(nodes, CREDIT_CURVE_NAME, marketData, rates, REF_DATA);
  }
  ...
  public void parellelCs01Test() {
    ...
    PointSensitivities point = PRICER.presentValueOnSettleSensitivity(CDS1, RATES_PROVIDER, REF_DATA).build();
  ...
  public void bucketedCs01Test() {
    double[] expectedFd = new double[] {
        0.02446907003406107, 0.1166137422736746, 0.5196553952424576, 1.4989046391578054, 3.5860718603647483, 4233.77162264947, 0.0};
  ...
  public void bucketedCs01SingleNodeCurveTest() {
    ...
    CdsCurveNode node = CdsCurveNode.ofParSpread(DatesCdsTemplate.of(START, END2, CDS_CONV), quoteId, LEGAL_ENTITY);
    ...
    LegalEntitySurvivalProbabilities creditCurve =
        BUILDER.calibrate(ImmutableList.of(node), CREDIT_CURVE_NAME, marketData, ratesProviderNoCredit, REF_DATA);
