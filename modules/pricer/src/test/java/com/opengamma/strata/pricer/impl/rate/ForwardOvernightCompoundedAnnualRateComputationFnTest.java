...
public class ForwardOvernightCompoundedAnnualRateComputationFnTest {
  ...
  public void test_rateForwardSensitivity() {
    ...
    when(mockRates.periodRatePointSensitivity(BRL_OBS[1], FIXING_END_DATE)).thenReturn(
        rateSensitivity);
    ...
    when(mockRatesUp.periodRate(BRL_OBS[1], FIXING_END_DATE)).thenReturn(
        rateCmp + EPS_FD);
    ...
    when(mockRatesDw.periodRate(BRL_OBS[1], FIXING_END_DATE)).thenReturn(
        rateCmp - EPS_FD);
  ...
  public void test_rateForwardOneFixing() {
    ...
    double rateExpected = (Math.pow(1.0d + FIXING_RATES[1], afKnown) * (1.0 + rateCmp * af) - 1.0d)
        / (afKnown + af);
    for (int loopvaldate = 0; loopvaldate < 2; loopvaldate++) {
      when(mockRates.getValuationDate()).thenReturn(valuationDate[loopvaldate]);
      double rateComputed = OBS_BRL_FWD_ONCMP.rate(ro, DUMMY_ACCRUAL_START_DATE, DUMMY_ACCRUAL_END_DATE, simpleProv);
