...
public class DiscountingBondFutureTradetPricerTest {
  ...
  public void test_presentValueSensitivity() {
    ...
    assertThat(computed.equalWithTolerance(expected, 10.0 * EPS * NOTIONAL * QUANTITY)).isTrue();
  }
  ...
  public void regression() {
    CurrencyAmount pv = TRADE_PRICER.presentValue(FUTURE_TRADE, PROVIDER, SETTLE_PRICE);
    assertThat(pv.getAmount()).isCloseTo(-2937800.66334416, offset(EPS * NOTIONAL * QUANTITY));
    ...
    assertThat(actualIssuer.equalWithTolerance(expectedIssuer, TOL * NOTIONAL * QUANTITY)).isTrue();
    ...
    assertThat(actualRepo.equalWithTolerance(expectedRepo, TOL * NOTIONAL * QUANTITY)).isTrue();
  }
  ...
  public void regression_withZSpread_continuous() {
    CurrencyAmount pv = TRADE_PRICER.presentValueWithZSpread(
        FUTURE_TRADE, PROVIDER, SETTLE_PRICE, Z_SPREAD, CONTINUOUS, 0);
    assertThat(pv.getAmount()).isCloseTo(-7728642.649169521, offset(EPS * NOTIONAL * QUANTITY));
  }
  ...
  public void regression_withZSpread_periodic() {
    CurrencyAmount pv = TRADE_PRICER.presentValueWithZSpread(
        FUTURE_TRADE, PROVIDER, SETTLE_PRICE, Z_SPREAD, PERIODIC, PERIOD_PER_YEAR);
    assertThat(pv.getAmount()).isCloseTo(-7710148.864076961, offset(EPS * NOTIONAL * QUANTITY));
