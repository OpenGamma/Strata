...
/**
 * Test {@link NormalSwaptionExpiryStrikeVolatilities}.
 */
@Test
public class NormalSwaptionExpiryStrikeVolatilitiesTest {
  ...
  private static final DoubleArray STRIKE =
      DoubleArray.of(0.025, 0.03, 0.035, 0.04, 0.025, 0.03, 0.035, 0.04, 0.025, 0.03, 0.035, 0.04);
  ...
  static {
    List<SwaptionSurfaceExpiryStrikeParameterMetadata> list =
        new ArrayList<SwaptionSurfaceExpiryStrikeParameterMetadata>();
    int nData = TIME.size();
    for (int i = 0; i < nData; ++i) {
      SwaptionSurfaceExpiryStrikeParameterMetadata parameterMetadata =
  ...
  private static final double TEST_FORWARD = 0.033; // not used internally
  ...
  public void test_volatility_sensitivity() {
    ...
    for (int i = 0; i < NB_TEST; i++) {
      ...
      for (int j = 0; j < nData; ++j) {
        ...
        NormalSwaptionExpiryStrikeVolatilities provUp =
            NormalSwaptionExpiryStrikeVolatilities.of(CONVENTION, VAL_DATE_TIME, paramUp);
        NormalSwaptionExpiryStrikeVolatilities provDw =
            NormalSwaptionExpiryStrikeVolatilities.of(CONVENTION, VAL_DATE_TIME, paramDw);
        ...
        map.put(DoublesPair.of(TIME.get(j), STRIKE.get(j)), fd);
      }
      List<ParameterMetadata> list = sensi.getParameterMetadata();
      assertEquals(computed.size(), nData);
      for (int j = 0; j < list.size(); ++j) {
        SwaptionSurfaceExpiryStrikeParameterMetadata metadata =
            (SwaptionSurfaceExpiryStrikeParameterMetadata) list.get(i);
        double expected = map.get(DoublesPair.of(metadata.getYearFraction(), metadata.getStrike()));
  ...
  public void coverage() {
    NormalSwaptionExpiryStrikeVolatilities test1 =
        NormalSwaptionExpiryStrikeVolatilities.of(CONVENTION, VAL_DATE_TIME, SURFACE);
    coverImmutableBean(test1);
    NormalSwaptionExpiryStrikeVolatilities test2 =
        NormalSwaptionExpiryStrikeVolatilities.of(CONVENTION, VAL_DATE.atStartOfDay(ZoneOffset.UTC), SURFACE);
