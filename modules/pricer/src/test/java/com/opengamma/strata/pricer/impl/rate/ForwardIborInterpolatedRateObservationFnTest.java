...
public class ForwardIborInterpolatedRateObservationFnTest {
  ...
  public void test_rate() {
    ...
    assertEquals(built.get(ExplainKey.OBSERVATIONS).get().get(0).get(ExplainKey.OBSERVED_RATE), Optional.of(RATE3));
    ...
    assertEquals(built.get(ExplainKey.OBSERVATIONS).get().get(1).get(ExplainKey.OBSERVED_INDEX), Optional.of(GBP_LIBOR_6M));
    assertEquals(built.get(ExplainKey.OBSERVATIONS).get().get(1).get(ExplainKey.OBSERVED_RATE), Optional.of(RATE6));
  ...
  public void test_rateSensitivity() {
    ...
    when(mockProv.iborIndexRates(GBP_LIBOR_3M)).thenReturn(mockRates3M);
    when(mockProv.iborIndexRates(GBP_LIBOR_6M)).thenReturn(mockRates6M);
    when(mockRates3M.ratePointSensitivity(FIXING_DATE)).thenReturn(SENSITIVITY3);
    when(mockRates6M.ratePointSensitivity(FIXING_DATE)).thenReturn(SENSITIVITY6);
