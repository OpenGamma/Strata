...
public class CdsMarketQuoteConverterTest {
  ...
  private static final CdsMarketQuoteConverter CONV_MARKIT_FIX = new CdsMarketQuoteConverter(AccrualOnDefaultFormulae.MARKIT_FIX);
  ...
  // rates provider without credit cuve
  ...
  public void parSpreadQuoteTest(){
    ...
    TradeInfo info =
        TradeInfo.builder().tradeDate(TODAY).settlementDate(products.get(0).getSettlementDateOffset().adjust(TODAY, REF_DATA)).build();
