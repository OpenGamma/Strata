...
public class AnalyticSpreadSensitivityCalculator
  ...
  public static final AnalyticSpreadSensitivityCalculator DEFAULT =
      new AnalyticSpreadSensitivityCalculator(AccrualOnDefaultFormulae.ORIGINAL_ISDA);
  ...
  DoubleArray computedBucketedCs01(
    ...
    PointSensitivities pointPv = pricer.presentValueOnSettleSensitivity(trade, ratesProviderBase, refData).build();
    DoubleArray vLambda =
        ratesProviderBase.singleCreditCurveParameterSensitivity(pointPv, legalEntityId, currency).getSensitivity();
    for (int i = 0; i < nBucket; i++) {
      PointSensitivities pointSp = pricer.parSpreadSensitivity(bucketCds.get(i), ratesProviderBase, refData).build();
