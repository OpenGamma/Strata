...
public final class FloatingRateCalculation
  ...
  /**
   * The first floating rate, with a 5% rate expressed as 0.05, optional.
   * <p>
   ...
   * Other calculation elements, such as gearing or spread, do not apply.
  ...
  ImmutableList<AccrualPeriod> createAccrualPeriods(Schedule schedule) {
    ...
    List<Double> notionals = notional.getAmount().resolveValues(schedule.getPeriods());
    ...
    for (int i = 0; i < schedule.size(); i++) {
      ...
      if (resolvedRates.get(i) != null) {
        // TODO: check if initialRate has gearing/spread applied or not
  ...
  private List<Double> resolveFixedRates(int size, boolean hasInitialStub, boolean hasFinalStub) {
    ...
    if ((hasInitialStub && initialStub.getRate() != null) ||
        (hasFinalStub && finalStub.getRate() != null) ||
  ...
  // resolves any varying indices
  private List<RateIndex> resolveIndices(int size, boolean hasInitialStub, boolean hasFinalStub) {
    // initialStub and finalStub are not-null at this point
    if ((hasInitialStub && initialStub.getStartIndex() != null) ||
        (hasFinalStub && finalStub.getStartIndex() != null)) {
      RateIndex[] rates = new RateIndex[size];
      if (hasInitialStub) {
        rates[0] = initialStub.getStartIndex();
        if (initialStub.getEndIndex() != null) {
          throw new UnsupportedOperationException();
        }
      }
      if (hasFinalStub) {
        rates[rates.length - 1] = finalStub.getEndIndex();
        if (finalStub.getEndIndex() != null) {
          throw new UnsupportedOperationException();
        }
      }
      return Arrays.asList(rates);
    }
    return Collections.nCopies(size, index);
  }
  ...
  //-----------------------------------------------------------------------
  /**
   * Gets the first floating rate, with a 5% rate expressed as 0.05, optional.
   * <p>
   * In certain circumstances two counterparties agree the rate of the first fixing when the contract starts.
   * The rate is applicable for the first reset period of the first <i>regular</i> accrual period.
   * It is used in place of an observed fixing.
